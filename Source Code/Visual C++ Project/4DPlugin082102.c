...
void sys_ShellExecute(PA_PluginParameters params)
{
	...
	INT	 howToShow; // WJF 6/30/16 Win-21 INT_PTR -> INT
	//char *pChar;
...
BOOL reviewProcesses(HANDLE hPSapiDLL, LPFNENUMPROC lpfnEnumProc, LPFNENUMPROCMODS lpfnEnumProcMods,
	LPFNGETMODFNAME lpfnGetModFName, char *pAppName, BOOL bUseToolHelp)
{
	BOOL					returnValue = 0; // WJF 6/30/16 Win-21 LONG_PTR -> BOOL
...
void gui_SelectColor(PA_PluginParameters params)
{
	...
	LONG				rParam, gParam, bParam, returnValue = 0, i, hasCustom; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
...
{
	LONG					position_len = 0, tbRestriction = 0; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
	LONG					trackingRestriction = 0, processNbr; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
	char					position[2];
	INT						theChar = 0; // WJF 6/24/16 Win-21 Initialize to 0 // WJF 6/30/16 Win-21 INT_PTR -> INT
	...
	if ((tbRestriction != 0) && (strpbrk(position, "ltrb") != NULL)) {
		...
		theChar = position[0]; // WJF 6/30/16 Win-21 Removed cast
	}
...
{
	short returnValue;
	DWORD connectionType; // WJF 6/24/16 Win-21 LONG_PTR -> DWORD
...
void gui_GetSysColor(PA_PluginParameters params)
{
	...
	INT displayElement;  // WJF 6/30/16 Win-21 INT_PTR -> INT
	LONG retRValue, retGValue, retBValue; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
	DWORD dwResult;
...
{
	LONG returnValue; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
	INT displayElement[1]; // WJF 6/24/16 Win-21 INT_PTR -> INT
	COLORREF rgbValue[1];
...
void sys_SetEnv(PA_PluginParameters params)
{
	...
	returnValue = !(_putenv(envName)); // WJF 6/24/16 Win-21 putenv -> _putenv
...
{
	LONG returnValue, regKey, retErr, arraySize, expandDataSize; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
	DWORD dataSize = 0; // WJF 6/24/16 Win-21 LONG_PTR -> DWORD
	LONG i, len; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
	char regSub[MAXBUF];
	char regName[MAXBUF];
	...
	if (retErr == ERROR_SUCCESS){
		...
		if (retErr == ERROR_SUCCESS){
			...
			case REG_EXPAND_SZ:
				returnDataBuffer = malloc(dataSize);
				retErr = RegQueryValueEx(hOpenKey, regName, NULL, NULL, (LPBYTE)returnDataBuffer, &dataSize); // WJF 6/24/16 Win-21 Casting to LPBYTE
			...
			case REG_MULTI_SZ:
				...
				if (retErr == ERROR_SUCCESS)
				{
					...
					{
						len = (LONG)strlen(ptrData); // WJF 6/30/16 Win-21 Cast to LONG
			...
			case REG_SZ:
				returnDataBuffer = malloc(dataSize); // WJF 7/13/16 Win-21 Removed typecasting on malloc to follow C best practices
				retErr = RegQueryValueEx(hOpenKey, regName, NULL, NULL, (LPBYTE)returnDataBuffer, &dataSize); // WJF 6/24/16 Win-21 Casting to LPBYTE
...
{
	LONG language; // WJF 6/30/16 Win-21 LONG_PTR -> LONG
	// Get the function parameters.
...
void sys_PrintDirect2Driver(PA_PluginParameters params)
{
	...
	INT iErrCode = 0;                 // Int to hold the error code. // WJF 6/30/16 Win-21 INT_PTR -> INT
	...
	if (ret != 0)
	{
		...
		if (iErrCode == 0)
		{
			if (!TextOut(pd.hDC, 1, 1, data, (INT)strlen(data))) // WJF 6/30/16 Win-21 Casting to INT
			{
				iErrCode = GetLastError();
			}
		}
		...
		PA_ReturnLong(params, iErrCode); // WJF 6/30/16 Win-21 Removed cast
	}
	else {
		PA_ReturnLong(params, GetLastError()); // WJF 6/30/16 Win-21 Removed cast
	}	// end if
}//end function
...
void sys_KillProcessByName(PA_PluginParameters params)
{
	...
	// WJF 12/17/15 Win-7
	processName = malloc(dwProcessName + 1); // WJF 7/13/16 Win-21 Removed typecasting on malloc to follow C best practices
	if (processName != NULL){
...
void sys_KillProcessByID(PA_PluginParameters params)
{
	...
	LONG lPID;                         // Integer to hold the processID ($1) // WJF 6/30/16 Win-21 INT_PTR -> LONG
...
void sys_EnumProcesses(PA_PluginParameters params)
{
	...
	INT iSize = 0;                    // Int to control size of array // WJF 6/30/16 Win-21 INT_PTR -> INT
