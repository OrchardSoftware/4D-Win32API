...
typedef	F_PASCAL_PTR(void,*IterateFilterProcPtr) (const CInfoPBRec * const cpbPtr,
											  Boolean *quitFlag,
											  void *yourDataPtr);
...
F_PASCAL(OSErr)	IterateDirectory(short vRefNum,
								 long dirID,
								 StringPtr name,
...
F_PASCAL(OSErr)	FSpIterateDirectory(const FSSpec *spec,
									unsigned short maxLevels,
									IterateFilterProcPtr iterateFilter,
									void *yourDataPtr);
/*	¦ Iterate (scan) through a directory's content.
